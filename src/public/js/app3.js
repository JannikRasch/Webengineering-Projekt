...
.then(data1 => {
    let parser = new DOMParser() //Parser zum Verarbeiten der xml Daten
    let news1 = parser.parseFromString(data1, "application/xml")
    console.log(news1)
    buildNews1(news1) //Aufruf der Funktion, die die Daten aus der xml bekommt
})
...
.then(data2 => {
    ...
    console.log(news2)
    buildNews2(news2) //Aufruf der Funktion, die die Daten aus der xml bekommt
})
...
function buildNews1(x){
    ...
    */
    ...
    //Titel werden aus den xml Daten geholt
    let titles = x.getElementsByTagName('title')
    ...
    //Datum wird aus den xml Daten geholt
    ...
    //Texte werden aus den xml Daten geholt
    ...
    //links zu den Artikeln werden aus den xml Daten geholt
    ...
    //Schleife, die die Url der Bilder aus dem CData Teil der xml liest
    for(let i=0;i<10;i++){
...
//Funktion für den zweiten Feed ähnlich aufgebaut wie die erste Funktion
